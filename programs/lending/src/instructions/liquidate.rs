...
use super::calculate_accured_interest;
...
pub fn process_liquidate(ctx: Context<Liquidate>)->Result<()>{
    ...
    match ctx.accounts.collateral_mint.to_account_info().key(){
        key if key == user.usdc_address =>{
            let new_usdc = calculate_accured_interest(user.deposited_usdc, collateral_bank.interest_rate, user.last_updated)?;
            total_collateral = usdc_price.price as u64 * new_usdc;
            let new_sol = calculate_accured_interest(user.borrowed_sol, collateral_bank.interest_rate, user.last_updated_borrow)?;
            total_borrowed = sol_price.price as u64 * new_sol;
        }, 
        _=>{
            let new_sol = calculate_accured_interest(user.deposited_sol, collateral_bank.interest_rate, user.last_updated)?;
            total_collateral = sol_price.price as u64 * new_sol;
            let new_usdc = calculate_accured_interest(user.borrowed_usdc, borrowed_bank.interest_rate, user.last_updated_borrow)?;
    ...
    token_interface::transfer_checked(cpi_ctx_to_liquidator, liquidator_amount, decimals)?;
